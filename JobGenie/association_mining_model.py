...
support = float(input("Please enter support value in %: "))
confidence = float(input("Please enter confidence value in %: "))

C1 = {}
transactions = 0
D = []
T = []
with open("skill2vec_50k.csv", "r", encoding='utf-8') as f:
    for line in f:
        T = []
        transactions += 1
                ...
                for item in r:
                    ...
                    if 100*inc2/inc1 >= confidence:
                        for index in l:
                            if index not in s:
                                m.append(index)
                        print("Rule#  %d : %s ==> %s %d %d" %
                              (num, s, m, 100*inc2/len(D), 100*inc2/inc1))
                        num += 1
